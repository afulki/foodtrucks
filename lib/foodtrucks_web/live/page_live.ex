defmodule FoodtrucksWeb.PageLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket), do: Process.send_after(self(), :populate_pins, 1000)
  ...
  def handle_event("add_trucks_open_today", _params, socket) do
    new_socket =
      Query.open_today()
      |> Enum.reduce(socket, fn truck, socket ->
        push_event(socket, "new_truck", %{truck: truck_to_map_data(truck)})
      end)
  ...
  @impl true
  def handle_info(:populate_pins, socket) do
    new_socket =
      Query.all()
      |> Enum.reduce(socket, fn truck, socket ->
        push_event(socket, "new_truck", %{truck: truck_to_map_data(truck)})
